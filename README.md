...
- **CN:** 使用浏览器或 Windows 工具编辑图片和 GIF，保存为项目自定义的 `PMX1`（Pixel Matrix eXchange）动画文件（不是 MMD 的 PMX 模型），再通过局域网 HTTPS 发给 ESP32；ESP32 将动画写入单独的 Flash 分区并从 RAM 播放。
- **EN:** Browser and Windows tools for editing images and GIFs, saving them in the project-specific `PMX1` (Pixel Matrix eXchange) content format (not MikuMikuDance's PMX model), sending them to an ESP32 over LAN HTTPS, and playing them on a 64x64 HUB75 matrix.
